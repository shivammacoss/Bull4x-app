...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    sendTemplateEmail('welcome', email, {
      ...
      supportEmail: emailSettings?.fromEmail || getSupportEmailFallback(),
      year: new Date().getFullYear().toString()
    })
...
router.post('/signup', async (req, res) => {
  try {
    const { firstName, email, phone, countryCode, address, password, adminSlug, referralCode } = req.body
    ...
    // Signup email OTP disabled — simple email/password registration (mobile + web).
    // To re-enable: uncomment below and restore OTP flow on the client.
    // const otpEnabled = await isOTPEnabled()
    // if (otpEnabled) {
    ...
    if (referralCode) {
      ...
      if (referringIB) {
        ...
        console.log(`[Signup] User ${emailNorm} referred by IB ${referringIB.firstName} (${referralCode})`)
      }
    }
    ...
    // Welcome email after response (never block signup on SMTP / template DB)
    if (!otpEnabled) {
      setImmediate(async () => {
        try {
          const emailSettings = await EmailSettings.findOne()
          const baseUrl = getPublicWebUrl()
          await sendTemplateEmail('welcome', emailNorm, {
            firstName: user.firstName,
            email: user.email,
            accountId: '—',
            accountType: 'Client',
